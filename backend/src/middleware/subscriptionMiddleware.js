    ...
    try {
      ...
      if (business.subscriptionPlan === 'BASIC') {
        if (action === 'CREATE_LOCATION') {
          return res.status(403).json({
            error: 'El Plan Básico (18€/mes) permite 1 sola sede. Actualiza a Plan Pro para sedes ilimitadas.',
            requiresUpgrade: true,
            currentLimit: 1
          });
        }
        ...
        if (action === 'WHATSAPP_CONNECT') {
          return res.status(403).json({
            error: 'La automatización de mensajes por WhatsApp 2 vías requiere el Plan Pro (25€/mes).',
            requiresUpgrade: true
          });
        }
        ...
        if (action === 'CREATE_APPOINTMENT') {
          ...
          if (monthlyAppointmentsCount >= 40) {
            return res.status(403).json({
              error: 'Has alcanzado el límite de 40 citas este mes del Plan Básico (18€/mes). Actualiza a Plan Pro para citas ilimitadas.',
              requiresUpgrade: true,
              currentLimit: 40
            });
          }
