...
import {
  ...
  FloatingInput, Button, Badge, FieldGroup, Field, FieldLabel, Skeleton,
...
export default function MessagesSection({ businessId, profileName, setToast }: MessagesSectionProps) {
  const [whatsappStatus, setWhatsappStatus] = useState("DISCONNECTED");
  const [qrCode, setQrCode] = useState<string | null>(null);
  const [loadingQr, setLoadingQr] = useState(false);
  const [pollingActive, setPollingActive] = useState(false);

  const [templates, setTemplates] = useState<MessageTemplates>({ welcomeMessage: "", reminderMessage: "" });
  ...
  const fetchTemplates = useCallback(() => {
    ...
    fetch(`/api/backend/whatsapp/templates?businessId=${businessId}`)
      ...
      .then((data) => {
        if (data && !data.error) {
          setTemplates({ welcomeMessage: data.welcomeMessage || "", reminderMessage: data.reminderMessage || "" });
        }
      })
      .catch(() => {});
  }, [businessId]);
  ...
  const handleConnectWhatsapp = () => {
    ...
    fetch("/api/backend/whatsapp/init", {
      ...
      .then((res) => res.json())
      .then(() => { setWhatsappStatus("WAITING_QR"); setPollingActive(true); setLoadingQr(false); })
  ...
  const handleDisconnectWhatsapp = () => {
    ...
    fetch("/api/backend/whatsapp/disconnect", {
      ...
      .then((res) => res.json())
      .then(() => {
        setWhatsappStatus("DISCONNECTED"); setQrCode(null); setPollingActive(false);
  ...
  const handleSaveTemplates = (e: React.FormEvent) => {
    ...
    fetch("/api/backend/whatsapp/templates", {
      ...
      .then((res) => { if (!res.ok) throw new Error("Failed to save templates"); return res.json(); })
      .then(() => {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-gutter animate-in fade-in duration-200">
      ...
      <Card className="sm:col-span-2 lg:col-span-5 flex flex-col justify-between min-h-0 sm:min-h-[420px]">
        <div>
          <CardHeader className="pb-4">
            <CardTitle className="text-primary flex items-center gap-2">
              <MessageSquare /><span>Canal de WhatsApp</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  {qrCode ? (
                    <div className="flex flex-col items-center bg-white p-4 rounded-md border border-outline-variant shadow-sm max-w-[240px]">
                      <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(qrCode)}`}
                        alt="WhatsApp QR Code" className="w-[180px] h-[180px]" />
                      <span className="text-[11px] font-medium text-on-surface-variant mt-2 text-center">Código QR de sincronización</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center bg-white p-4 rounded-md border border-outline-variant shadow-sm w-[212px] h-[224px]">
                      ...
                      <span className="text-[11px] font-medium text-on-surface-variant mt-2 text-center animate-pulse">Generando QR...</span>
                    </div>
                  )}
        ...
        <CardFooter className="pt-0">
          {whatsappStatus === "CONNECTED" ? (
            <Button variant="outline" size="lg" onClick={handleDisconnectWhatsapp}
            ...
            <Button variant="primary" size="lg" onClick={handleConnectWhatsapp} disabled={loadingQr}
              className="w-full py-3 flex items-center justify-center gap-2 active:scale-[0.98] font-medium">
              {loadingQr ? <><Loader2 className="animate-spin" /><span>Iniciando...</span></> : <span>Vincular WhatsApp</span>}
            </Button>
          )}
        </CardFooter>
      </Card>
      ...
      <Card className="sm:col-span-2 lg:col-span-7 flex flex-col justify-between">
        <div>
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <CardTitle className="text-primary flex items-center gap-2">
              <Send /><span>Plantillas de Mensajería</span>
            </CardTitle>
            {!isEditingTemplates ? (
              <Button variant="ghost" onClick={() => setIsEditingTemplates(true)}
              ...
              <div className="flex gap-3">
                <Button variant="ghost" onClick={() => { setIsEditingTemplates(false); fetchTemplates(); }}
                ...
                <Button variant="primary" size="md" onClick={handleSaveTemplates} disabled={savingTemplates}
            ...
            <form className="flex flex-col gap-4 sm:gap-6">
              <FieldGroup className="gap-4 sm:gap-6">
                <Field>
                  ...
                  <textarea disabled={!isEditingTemplates} rows={3} value={templates.welcomeMessage}
                ...
                <Field>
                  ...
                  <textarea disabled={!isEditingTemplates} rows={3} value={templates.reminderMessage}
              ...
              <div className="bg-surface-container-low p-4 rounded-md border border-outline-variant/50">
                ...
                <div className="flex flex-wrap gap-2">
                  {["{{clientName}}", "{{appointmentDate}}", "{{appointmentTime}}", "{{businessName}}"].map((v) => (
                    <span key={v} className="bg-surface-container-lowest border border-outline-variant text-[11px] px-2 py-1 rounded font-mono select-all cursor-pointer"
                      title={v === "{{clientName}}" ? "Nombre del cliente" : v === "{{appointmentDate}}" ? "Fecha de la cita" : v === "{{appointmentTime}}" ? "Hora de la cita" : "Nombre comercial"}>
                      {v}
                    </span>
                  ))}
                </div>
