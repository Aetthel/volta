...
import {
  ...
  FloatingInput, Button, Badge, FieldGroup, Field,
...
export default function ProfileSection({ profile, setProfile, setToast }: ProfileSectionProps) {
  ...
  const [userProfileData, setUserProfileData] = useState<{ id?: string; createdAt?: string } | null>(null);
          ...
          {!isEditingProfile ? (
            ...
            <div className="flex gap-3">
              <Button variant="ghost" onClick={() => {
                setIsEditingProfile(false);
                if (session?.user) setUserForm({ name: session.user.name || "", email: session.user.email || "", password: "" });
              ...
              <Button variant="primary" size="md" onClick={handleSaveProfile} disabled={savingProfile}
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
            <div className="flex items-center gap-6">
              <div className="w-24 h-24 shrink-0 relative">
                <div onClick={isEditingProfile ? () => workerPhotoInputRef.current?.click() : undefined}
                  className={cn("w-24 h-24 rounded-full overflow-hidden border-2 border-primary/30 bg-surface-container shadow-sm flex items-center justify-center transition-all duration-200 relative group/avatar",
                    isEditingProfile ? "cursor-pointer hover:opacity-90" : "cursor-default")}>
                  {profile.workerPhoto && profile.workerPhoto !== DEFAULT_AVATAR ? (
                    <img src={profile.workerPhoto} alt="Foto de perfil" className="w-full h-full object-cover" />
                  ) : (
                ...
                <input type="file" ref={workerPhotoInputRef} onChange={handleWorkerPhotoChange} accept="image/*" className="hidden" />
              </div>
              <div className="flex flex-col gap-1">
                <h2 className="font-display text-2xl font-semibold text-on-surface">{userForm.name || "Sin nombre"}</h2>
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="default" className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full">
          ...
          {isEditingProfile ? (
            <FieldGroup className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
              <Field>
                <FloatingInput id="user-name" label="Nombre Completo" type="text" required value={userForm.name}
                  onChange={(e) => setUserForm({ ...userForm, name: e.target.value })} />
              </Field>
              <Field>
                <FloatingInput id="user-email" label="Correo Electrónico" type="email" required value={userForm.email}
                  onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} />
              </Field>
              <Field>
                <FloatingInput id="user-password" label="Nueva Contraseña (opcional)" type="password" value={userForm.password}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
              <div>
                <span className="font-label-sm text-label-sm text-on-surface-variant font-semibold uppercase tracking-wider block mb-1">Nombre Completo</span>
                <p className="font-body-lg text-body-lg font-semibold text-on-surface">{userForm.name || "Sin nombre"}</p>
              </div>
              <div>
                <span className="font-label-sm text-label-sm text-on-surface-variant font-semibold uppercase tracking-wider block mb-1">Correo Electrónico</span>
                <p className="font-body-lg text-body-lg font-semibold text-on-surface">{userForm.email || "Sin email"}</p>
              </div>
              <div>
                <span className="font-label-sm text-label-sm text-on-surface-variant font-semibold uppercase tracking-wider block mb-1">Fecha de Ingreso</span>
                <p className="font-body-lg text-body-lg font-semibold text-on-surface">{formatProfileDate(userProfileData?.createdAt || "")}</p>
              </div>
            </div>
          )}
        </CardContent>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="border border-outline-variant rounded-md p-3 sm:p-4 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div>
                  <p className="font-body-lg text-body-lg font-semibold text-on-surface">Contraseña</p>
                  <p className="font-body-md text-body-md text-on-surface-variant">Actualizada hace 3 meses</p>
                </div>
              </div>
              <Button variant="ghost" className="text-primary hover:text-primary-container font-label-lg text-label-lg transition-all hover:underline p-0 shadow-none active:scale-100 font-medium">
                Cambiar
              </Button>
            </div>
            <div className="border border-outline-variant rounded-md p-3 sm:p-4 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div>
                  <p className="font-body-lg text-body-lg font-semibold text-on-surface">Verificación en dos pasos</p>
                  <p className="font-body-md text-body-md text-primary font-semibold">Activada</p>
                </div>
              </div>
              <Button variant="ghost" className="text-error hover:text-error/80 font-label-lg text-label-lg transition-all hover:underline p-0 shadow-none active:scale-100 font-medium">
                Desactivar
              </Button>
            </div>
          </div>
