...
import {
  Button, FieldGroup, Field, FieldLabel, FloatingInput, InlineSelect,
...
export default function WorkerModal({ isOpen, onClose, onSave, formData, setFormData, errorMsg, isEditing }: WorkerModalProps) {
        ...
        <form onSubmit={onSave} className="p-6 flex flex-col gap-6">
          ...
          <FieldGroup>
            <Field>
              ...
              <FloatingInput id="workerName" label="Nombre y Apellidos" value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })} icon={User} required />
            </Field>
            <Field>
              <FieldLabel htmlFor="workerEmail">Correo Electrónico</FieldLabel>
              <FloatingInput id="workerEmail" label="correo@empresa.com" type="email" value={formData.email}
                onChange={(e) => setFormData({ ...formData, email: e.target.value })} icon={Mail} required />
            </Field>
            <Field>
              <FieldLabel htmlFor="workerPassword">
                Contraseña {isEditing && <span className="text-on-surface-variant/50 font-normal">(dejar en blanco para mantener)</span>}
              </FieldLabel>
              <FloatingInput id="workerPassword" label={isEditing ? "Nueva contraseña (opcional)" : "Mínimo 6 caracteres"}
                type="password" value={formData.password}
                onChange={(e) => setFormData({ ...formData, password: e.target.value })} icon={Key} required={!isEditing} />
            </Field>
            <Field>
              <FieldLabel htmlFor="workerRole">Rol de Usuario</FieldLabel>
              <InlineSelect id="workerRole" label="Seleccionar rol" value={formData.role}
                onChange={(val) => setFormData({ ...formData, role: val as "JEFE" | "EMPLEADO" })}
                options={[{ value: "EMPLEADO", label: "Empleado (Staff)" }, { value: "JEFE", label: "Jefe / Encargado" }]}
                variant="outlined" />
            </Field>
          </FieldGroup>
          ...
          <div className="flex items-center justify-end gap-3 mt-4 border-t border-outline-variant/50 pt-4">
            ...
            <Button type="submit" variant="primary" size="md" className="px-5 py-2.5 active:scale-95 font-medium">
