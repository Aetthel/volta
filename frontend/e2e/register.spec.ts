...
test.describe("Registration E2E Flow", () => {
  test("user can navigate through register steps 1 to 4", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /¿A qué sector pertenece tu negocio\?/i })).toBeVisible();
    await page.getByText("Peluquería").click();
    await page.getByRole("button", { name: "Siguiente" }).click();

    // Step 2: Business details
    await expect(page.getByRole("heading", { name: /Detalles de tu establecimiento/i })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /Crea tu cuenta de administrador/i })).toBeVisible();
