...
- **[Risk]**: Portals might render text out of Next.js hydration context.
  - *Mitigation*: Ensure components only mount on the client side (`mounted` check or checking if `window` exists).
- **[Risk]**: Scrolling the screen while the menu is open could detach the menu from the target element.
  - *Mitigation*: Automatically register a global scroll/resize listener that closes the menu.
