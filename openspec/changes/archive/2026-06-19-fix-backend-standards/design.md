...
- **[Risk]**: Client applications or direct scripts could attempt to forge downstream header authentication variables.
  - _Mitigation_: The backend requires the shared static secret `API_KEY` via `x-api-key` header (enforced by the `authenticate` middleware). The backend MUST NOT accept public requests that do not pass key validation.
- **[Risk]**: Querying related services and clients for all appointments in dashboard stats could reduce performance for large datasets.
  - _Mitigation_: Query optimizations are applied by using Prisma's `include` filters to resolve relations in a single database round-trip.
