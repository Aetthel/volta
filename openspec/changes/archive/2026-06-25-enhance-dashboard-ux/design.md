...
- **[Risk] Descubribilidad del menú contextual:** Los usuarios podrían no saber que pueden hacer clic derecho o mantener pulsado para gestionar el estado de una cita.
  - *Mitigación:* Añadiremos la clase `cursor-context-menu` a la tarjeta de la cita, un sutil indicador visual (tres puntos verticales `MoreVertical` en la esquina derecha que disparen el menú) y un estado de hover diferenciado para invitar a la interacción.
- **[Risk] Rendimiento en listas largas:** La carga de muchos nodos de ContextMenu puede saturar el DOM.
  - *Mitigación:* El dashboard de inicio está limitado únicamente a las citas de hoy (normalmente < 15-20 registros), por lo que el impacto en el DOM es despreciable.
