...
- **[Risk] Middleware redirection loop or access denial**: If middleware intercepts `/` and redirects back to `/login`, visitors will not see the landing page.
  - *Mitigation*: Verify and update next-auth/middleware config to allow unauthenticated access to the root path `/`.
- **[Risk] Duplicate branding references**: The application layout might define a header or footer that conflicts with the custom SaaS design.
  - *Mitigation*: Create a custom page structure inside `/page.tsx` that bypasses shared internal headers/footers, or wrap the internal layouts under route groups (e.g., `(dashboard)/layout.tsx`). Let's check `frontend/app/layout.tsx` for layout encapsulation.
