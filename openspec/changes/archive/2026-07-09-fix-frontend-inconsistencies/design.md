...
- **Risk:** Slow APIs might keep the skeleton flashing longer.
  - *Mitigation:* Ensure queries are efficient (indexed Prisma queries).
- **Risk:** Double render triggers.
  - *Mitigation:* Keep state management clean, fetching once on mount in standard `useEffect` hooks.
