...
- **[Risk] DB Polling Overhead** → *Mitigation:* The frontend will poll `/api/alerts` every 30 seconds. To minimize database load, the endpoint queries PostgreSQL utilizing the index on `userId`.
- **[Risk] General broadcast overhead** → *Mitigation:* System-wide broadcasts (from the admin view) will spawn rows for all targeted users. In production, this can be offloaded to a background worker to avoid blocking the main server thread.
